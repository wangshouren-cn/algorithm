import { TreeNode, isSameTree, printTree } from "../utils/Tree";
...
*/
function getTwoErrorsNode(head: TreeNode) {
  if (!head) return [];
  const stack = new Stack<TreeNode>();
  const errors: TreeNode[] = [];
  let pre = null;
...
function getTwoParents(head: TreeNode, e1: TreeNode, e2: TreeNode) {
  const stack = new Stack<TreeNode>();
...
const RIGHT = "right";
type D = "left" | "right";
function recoverTree(head: TreeNode) {
  const [e1, e2] = getTwoErrorsNode(head);
  let [e1P, e2P] = getTwoParents(head, e1, e2);
  const e1L = e1.left,
    e1R = e1.right,
    e2L = e2.left,
    e2R = e2.right;
  ...
  let e1LN = e2L,
    ...
    e1PN = e2P,
    e2PN = e1P,
    d1: D = e2P?.left == e2 ? LEFT : RIGHT,
    d2: D = e1P?.left == e1 ? LEFT : RIGHT,
    returnNode = head;
...
function test(text: string, x1: number, x2: number) {
  ...
  nodes[5] = new TreeNode(5);
  nodes[6] = new TreeNode(6);
  nodes[7] = new TreeNode(7);
  nodes[8] = new TreeNode(8);
  ...
  const rightNode = JSON.parse(JSON.stringify(nodes[5]));
  nodes[x1].data = x2;
  nodes[x2].data = x1;
  let newNode = recoverTree(nodes[5]);
  console.log("通过：", isSameTree(rightNode, newNode));
}
